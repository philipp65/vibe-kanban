...
const GlabCliSetupDialogImpl = create<NoProps>(() => {
  ...
  return (
    <Dialog open={modal.visible} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t('settings:integrations.gitlab.cliSetup.title')}</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-3">
          <p className="text-sm">
            {t('settings:integrations.gitlab.cliSetup.installLabel')}
          </p>
          <pre className="rounded bg-muted px-2 py-1 text-xs">brew install glab</pre>
          <p className="text-sm">
            {t('settings:integrations.gitlab.cliSetup.loginLabel')}
          </p>
          <pre className="rounded bg-muted px-2 py-1 text-xs">glab auth login</pre>
        </div>
        <DialogFooter>
...
export const GlabCliSetupDialog = defineModal<void, void>(GlabCliSetupDialogImpl);
import {
  Dialog,
  DialogContent,
  DialogHeader,
